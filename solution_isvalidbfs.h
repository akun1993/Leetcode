...
public:
    ...
    bool dfs(TreeNode* root,int dir ,TreeNode* left_root,TreeNode* right_root)
    {
        ...
        if(left_root && root->val >= left_root->val) return false;

        if(right_root && root->val <= right_root->val) return false;

        bool res = true;
        if(dir == LEFT_LEFT)
        {
            if(res) res = dfs(root->left,LEFT_LEFT,root,right_root);
            if(res) res = dfs(root->right,LEFT_RIGHT,left_root,root);
        }

        if(dir == LEFT_RIGHT)
        {
            if(res) res = dfs(root->left,RIGHT_LEFT,root,right_root);
            if(res) res = dfs(root->right,RIGHT_RIGHT,left_root,root);
        }
    ...
    bool dfsII(TreeNode* root,TreeNode* left_root,TreeNode* right_root)
    {
        ...
        if(res) res = dfsII(root->right,left_root,root);
        return res;
    }
    ...
    bool IsValidBfs(TreeNode* root)
    {
        ...
        if(root->right )
        {
            ...
            if(res) res = dfsII(root->right ,NULL,root);
        }
    ...
    void answer()
    {
        ...
        vector<int> case_4 = {7,1,5,NULL,2,4,6,NULL,NULL,NULL,3};//Ԥ�ڣ�false
        ...
        for(int i = 0; i < (int)arr.size();++i)
        {
           ...
           // bool is_symmetric = tree.SymmetricIterator(head);
            ...
            if(res )
                cout<< endl <<"res is " << res->val << endl;
            else
                cout<< endl <<"res is NULL" << endl;
